    ...
    <div class="card-body">
        <form action="{{ route("admin.blog.update", [$blog->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <input type="text" id="slug" name="slug" class="form-control" value="{{ old('slug', isset($blog) ? $blog->slug : '') }}" required>
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                 <label for="name">Category</label>
                  <select name="category_id"  class="form-control select2" >
                    @foreach($category as $id => $categories)
                        <option value="{{ $id }}" {{ $blog->category_id == $id ? 'selected' : '' }}>{{ $categories }}</option>
                 ...
                 <label for="name">Author*</label>
                  <select name="author"  class="form-control select2"  required>
                    @foreach($author as $id => $authors)
                        <option value="{{ $id }}" {{ $blog->author_id == $id ? 'selected' : '' }}>{{ $authors }}</option>
                ...
                <input type="date" name="dates" class="form-control" value="{{ date("Y-m-d", strtotime($blog->created_at))}}" />
                   </div>
                
           
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                <label for="name">Description*</label>
                 <textarea  name="description" rows="4" cols="50" class="form-control">{{$blog->description}}
                     </textarea>
            ...
            <div class="col-md-12 pl-0">
                <img src="{{ URL::to('/') }}/uploads/blog/{{ $blog->image }}" class="img-thumbnail" width="350px" height="300px" />
            </div>
            @endif
           <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
            ...
            @if(@$blog->image1)
            <div class="col-md-12 pl-0">
                <img src="{{ URL::to('/') }}/uploads/blog/{{ $blog->image1 }}" class="img-thumbnail" width="400px" height="400px" />
            </div>
            @endif
            <div>
