        ...
        <form action="{{ route("admin.video.update", [$video->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            @method('PUT')
              ...
              <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <input type="text" id="title" name="title" class="form-control" value="{{ old('title', isset($video) ? $video->title : '') }}" required>
               
            </div>
            
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                 <label for="name">Page</label>
                 <input type="text" readonly id="page" name="page" class="form-control" value="{{ old('title', isset($video) ? $video->page : '') }}" required>  </div>
            ...
            <div class="col-md-12 pl-0">
                <img src="{{ URL::to('/') }}/public/uploads/vodeo_image/{{ $video->image }}" class="img-thumbnail" width="350px" height="300px" />
            </div>
            @endif
           <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                <label for="name">Video Link  </label>
            <input type="text" id="video" name="video" class="form-control" value="{{ old('title', isset($video) ? $video->video : '') }}" > 
            </div>
